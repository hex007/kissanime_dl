...
def printCapchaWarning():
    printClr("Warning: KissCartoon and KissAsian tend to ask for capcha after 5 video intervals.", Color.BOLD,
             Color.YELLOW)
                ...
                if openload is False:
                    if getBlogspotUrls(queuee, ur, ses, sleepy_increment * count + sleepy_time) == False:
                        if getOpenLoadUrls(queuee, ur, ses, sleepy_increment * count + sleepy_time) == False:
                            printClr(
                                    "Failed to find url. You may have to check capcha, or KissAnime may have changed video host.",
